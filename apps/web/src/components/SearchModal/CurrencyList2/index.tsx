...
import { scrollbarStyle } from 'components/SearchModal/CurrencyList2/index.css'
...
export const formatAnalyticsEventProperties2 = (
  token: Currency,
  index: number,
  data: any[],
  searchQuery: string,
 ...
 */
class CurrencyListSectionTitle extends CurrencyListRow {
  constructor(public readonly label: string) {
    super(undefined)
  }
}

export default function CurrencyList2({
  height,
  currencies,
  selectedCurrency,
  onCurrencySelect,
  ...
  const Row = useCallback(
    function TokenRow({ data, index, style }: TokenRowProps) {
      ...
      const { loading: getInvestorTokenAmountLoading, result: [getInvestorTokenAmount] = [] } = useSingleCallResult(
        ...
        [fundId ?? undefined, investor ?? undefined, row.currency?.wrapped.address],
      )
      ...
      if (isLoading) {
        ...
        return (
          <CurrencyRow2
            ...
            eventProperties={formatAnalyticsEventProperties2(token, index, data, searchQuery, isAddressSearch)}
            balance={balance()}
  ...
  const itemKey = useCallback((index: number, data: typeof currencies) => {
    ...
    return currencyListRowKey(currency) + `-${index}`
  }, [])
