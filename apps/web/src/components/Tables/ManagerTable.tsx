...
import { Manager } from 'data/Fund/managerData'
...
export function ManagerTable({ manager }: { manager: Manager[] }) {
  return (
    <TableWrapper data-testid="top-tokens-explore-table">
      <ManagerTable2 manager={manager} loading={manager ? false : true} />
    </TableWrapper>
  )
}
...
function ManagerTable2({
  ...
  const tokenTableValues = useMemo(
    () =>
      manager?.map((manager: Manager, i: number) => {
        return {
          index: i + 1,
          tokenDescription: (
            <Flex row gap="$gap8">
              <EllipsisText data-testid="manager-id">{manager.id.slice(0, 8)}...</EllipsisText>
              <TokenTableText>{manager.investor.slice(0, 6)}...</TokenTableText>
            </Flex>
          ),
          price: parseFloat(manager.principalUSD.toString()),
          testId: `manager-table-row-${manager.id}`,
          percentChange1hr: (
            <>
              <DeltaArrow delta={0} />
          ...
          fdv: parseFloat(manager.principalUSD.toString()),
          volume: parseInt(manager.currentUSD.toString()),
          sparkline: <SparklineContainer></SparklineContainer>,
          link: `/fund/${manager.fundId}/${manager.investor}`,
          analytics: {
            elementName: InterfaceElementName.TOKENS_TABLE_ROW,
            properties: {
              fund_id: manager.id,
              fund_manager: manager.investor,
              investor_count: manager.fundId,
              current_value: manager.currentUSD,
            },
          },
          linkState: {},
        }
