...
import { Investor } from 'data/Fund/investorData'
...
export function InvestorTable({ investor }: { investor: Investor[] }) {
  return (
    <TableWrapper data-testid="top-tokens-explore-table">
      <InvestorTable2 investor={investor} loading={investor ? false : true} />
    </TableWrapper>
  )
}
...
function InvestorTable2({
  ...
  const tokenTableValues = useMemo(
    () =>
      investor?.map((investor: Investor, i: number) => {
        return {
          index: i + 1,
          tokenDescription: (
            <Flex row gap="$gap8">
              <EllipsisText data-testid="investor-id">{investor.id.slice(0, 8)}...</EllipsisText>
              <TokenTableText>{investor.investor.slice(0, 6)}...</TokenTableText>
            </Flex>
          ),
          price: parseFloat(investor.principalUSD.toString()),
          testId: `investor-table-row-${investor.id}`,
          percentChange1hr: (
            <>
              <DeltaArrow delta={0} />
          ...
          fdv: parseFloat(investor.principalUSD.toString()),
          volume: parseInt(investor.currentUSD.toString()),
          sparkline: <SparklineContainer></SparklineContainer>,
          link: `/fund/${investor.fundId}/${investor.investor}`,
          analytics: {
            elementName: InterfaceElementName.TOKENS_TABLE_ROW,
            properties: {
              fund_id: investor.id,
              fund_investor: investor.investor,
              investor_count: investor.fundId,
              current_value: investor.currentUSD,
            },
          },
          linkState: {},
        }
